...
def Fit_spectrums(scan, expt, is_save=True):
    """
    ...
    if is_save:
        ...
        with open(expt.working_dir+scan.id+'/FitResults.csv', "w", newline='') as f:
    ...
    count=0
    for spectrum in scan.spectrums:
        ...
        def iter_cb(params, nb_iter, resid, *args, **kws):
            ...
            if nb_iter > expt.fitstuck_limit:
                scan.is_fitstuck = True

            if np.sum(spectrum)<10.:
                scan.is_fitstuck = True

            return scan.is_fitstuck
        ...
        #####################################################
        if is_save:
            with open(expt.working_dir+scan.id+'/FitResults.csv', 'a+', newline='') as f:
                writer = csv.writer(f,delimiter=expt.delimiter)
        ...
        fig.suptitle('Fit of spectrum %g/%g'%(count,(len(scan.spectrums)-1)), fontsize=14)
    ...
    # Results after fits
    scan.dparams_list = dparams_list

    FF.Plot_fit_results(scan, expt, spectrum_index=None, dparams_list=dparams_list, is_save=is_save)

    print('#####################################################')
    print('Results are saved in:\n%s'%(expt.working_dir+scan.id+'/FitResults.csv'))
